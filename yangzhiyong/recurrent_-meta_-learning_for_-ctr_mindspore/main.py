...
def eval_test(cur_status,sess,model,curlog):
    ...
    for _, epoch_data in our_Data_loader(USE_DATA,test_set,user_dict,item_dict, BATCH_SIZE):
        ...
        users, targets, targets_labels, hists, items_length,hists_y = epoch_data[0], epoch_data[1], epoch_data[2], epoch_data[3], \
                                                          epoch_data[4],epoch_data[5]

        cur_predicted, loss = model(users, targets, targets_labels, hists, items_length,hists_y)
        sum_loss += loss
        tot_realLabel.append(targets_labels.numpy())
        tot_preLabel.append(cur_predicted.numpy())
        cur_batch += 1
    tot_realLabel = np.concatenate(tot_realLabel, axis=0).tolist()
    tot_preLabel = np.concatenate(tot_preLabel, axis=0).tolist()
